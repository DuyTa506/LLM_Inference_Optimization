...
class KVCache:
    ...
    def truncate(self, length: int) -> None:
        ...
        Args:
            length: New length of the cache. Must be <= current_len.
        """
        if length > self.current_len:
            raise ValueError(f"Cannot truncate to {length}, current length is {self.current_len}")
...
class MultiLayerKVCache:
    ...
    def truncate(self, length: int) -> None:
        """Truncate all layer caches to specified length."""
        for cache in self.caches:
